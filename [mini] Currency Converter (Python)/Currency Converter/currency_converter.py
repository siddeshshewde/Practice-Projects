...
#Taking Input	
number = input("Enter a value which you want to convert : ")
currency = input("Enter the currency to which you want to convert the amount : ")

if number in currencyDict.keys():
	# calling a dictionary with a key returns the value associated to the key
	print (str(number) + 'USD represents ' + str(currency*currencyDict[currency]) + currency + '.')
else:
